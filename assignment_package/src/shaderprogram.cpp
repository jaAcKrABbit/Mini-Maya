...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      attrPos(-1), attrNor(-1), attrCol(-1),attrJointID(-1),attrWeight(-1),
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifCamPos(-1),
      unifOverallTransformMats(-1), unifBindMats(-1), context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrWeight = context->glGetAttribLocation(prog, "vs_Weight");

    unifModel      = context->glGetUniformLocation(prog, "u_Model");
    unifModelInvTr = context->glGetUniformLocation(prog, "u_ModelInvTr");
    unifViewProj   = context->glGetUniformLocation(prog, "u_ViewProj");
    unifCamPos     = context->glGetUniformLocation(prog, "u_CamPos");
    unifOverallTransformMats  = context->glGetUniformLocation(prog, "u_OverallTransformMats");
...
void ShaderProgram::setOverallTransformationMatrices(const std::vector<glm::mat4> &transforms) {
    // Tell OpenGL to use this shader program for subsequent function calls
    useMe();

    if(unifOverallTransformMats != -1) {
    // Pass a 4x4 matrix into a uniform variable in our shader
                    // Handle to the matrix variable on the GPU
    context->glUniformMatrix4fv(unifOverallTransformMats,
                    // How many matrices to pass
                       transforms.size(),
                    // Transpose the matrix? OpenGL uses column-major, so no.
                       GL_FALSE,
...
void ShaderProgram::setBindMatrices(const std::vector<glm::mat4> &bindMatrices) {
    // Tell OpenGL to use this shader program for subsequent function calls
    useMe();
